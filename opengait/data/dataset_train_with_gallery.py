...
class DataSet_train_with_gallery(tordata.Dataset):
    ...
    def __dataset_parser(self, data_config, training):
        ...
        def get_seqs_info_list_train(label_set, test_set):
            seqs_info_list = []
            dataset_name = data_config['dataset_name']
            if dataset_name == "Gait3D":
                ...
                for lab in test_set:
                    for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                        for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                            seq_info = [lab, typ, vie]
                            if '-'.join([lab, typ, vie]) in probe_set:
                                # print('-'.join([lab, typ, vie]))
                                continue
                            seq_path = osp.join(dataset_root, *seq_info)
                            seq_dirs = sorted(os.listdir(seq_path))
                            if seq_dirs != []:
                                ...
                                msg_mgr.log_debug(
                                    'Find no .pkl file in %s-%s-%s.' % (lab, typ, vie))
            if dataset_name == "CASIA-E":
                gallery_set = ['H-scene1-nm-1', 'H-scene1-nm-2', 'L-scene1-nm-1', 'L-scene1-nm-2']
                for lab in label_set:
                    for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                        for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                            seq_info = [lab, typ, vie]
            ...
            if dataset_name == "CASIA-B":
                gallery_set = ['nm-01', 'nm-02', 'nm-03', 'nm-04']
                for lab in label_set:
                    for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                        for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                            seq_info = [lab, typ, vie]
        ...
        def get_seqs_info_list_test(label_set):
            seqs_info_list = []
            for lab in label_set:
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
        ...
        self.seqs_info = get_seqs_info_list_train(
            train_set,test_set) if training else get_seqs_info_list_test(test_set)

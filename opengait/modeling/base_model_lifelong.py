...
class BaseModellifelong(MetaModel, nn.Module):
    """Base model.
    ...
    def __init__(self, cfgs, training):
        ...
        super(BaseModellifelong, self).__init__()
        ...
        self.to(device=torch.device(
            "cuda", self.device))
        self.current_step = cfgs['model_cfg']['current_step']
        if training:
            self.loss_aggregator = LossAggregator(cfgs['loss_cfg'])
            self.task_optimizer = self.get_optimizer(self.cfgs['task_optimizer_cfg'], self.Taskbone)
            self.graph_optimizer = self.get_optimizer(self.cfgs['graph_optimizer_cfg'], self.Graphbone)
            self.task_scheduler = self.get_scheduler(cfgs['task_scheduler_cfg'], self.task_optimizer)
            self.graph_scheduler = self.get_scheduler(cfgs['graph_scheduler_cfg'], self.graph_optimizer)
            self.step_start_iteration = self.engine_cfg['step_start_iteration'][self.current_step]
        self.train(training)
    ...
    def get_optimizer(self, optimizer_cfg, model):
    ...
    def save_ckpt(self, iteration):
        if torch.distributed.get_rank() == 0:
            ...
            checkpoint = {
                ...
                'graph_scheduler': self.graph_scheduler.state_dict(),
                'iteration': iteration}
    ...
    def _load_ckpt(self, save_name):
        ...
        # del model_state_dict['BNNecks.fc_bin.step:1']
        ...
        # print(checkpoint)
        if self.training:
            if not self.engine_cfg["task_optimizer_reset"] and 'task_optimizer' in checkpoint:
                self.task_optimizer.load_state_dict(checkpoint['task_optimizer'])
            else:
                self.msg_mgr.log_warning(
                    "Restore NO task_Optimizer from %s !!!" % save_name)
            if not self.engine_cfg["graph_optimizer_reset"] and 'graph_optimizer' in checkpoint:
                self.graph_optimizer.load_state_dict(checkpoint['graph_optimizer'])
            else:
                self.msg_mgr.log_warning(
                    "Restore NO graph_Optimizer from %s !!!" % save_name)
            if not self.engine_cfg["task_scheduler_reset"] and 'task_scheduler' in checkpoint:
                self.task_scheduler.load_state_dict(
                    checkpoint['task_scheduler'])
                print(self.task_scheduler)
                torch.optim.lr_scheduler.MultiStepLR
            else:
                self.msg_mgr.log_warning(
                    "Restore NO task_Scheduler from %s !!!" % save_name)
            if not self.engine_cfg["graph_scheduler_reset"] and 'graph_scheduler' in checkpoint:
                self.graph_scheduler.load_state_dict(
                    checkpoint['graph_scheduler'])
            else:
                self.msg_mgr.log_warning(
                    "Restore NO graph_Scheduler from %s !!!" % save_name)
    ...
    def train_step(self, loss_sum) -> bool:
        ...
        if self.engine_cfg['enable_float16']:
            ...
            #         print(name)
            self.Scaler.step(self.task_optimizer)
        ...
        self.iteration += 1
        # print(self.step_start_iteration)
        self.task_scheduler.step(self.iteration+self.step_start_iteration)
        self.graph_scheduler.step(self.iteration+self.step_start_iteration)
        return True
    ...
    @staticmethod
    def copy_model_and_frozen(model):
    ...
    def run_train(model):
        ...
        if model.current_step > 0:
            old_task_model = model.copy_model_and_frozen(model.Taskbone)
            old_graph_model = model.copy_model_and_frozen(model.Graphbone)
        else:
            old_task_model = None
        ...
        for inputs in model.train_loader:
            ...
            with autocast(enabled=model.engine_cfg['enable_float16']):
                retval = model(ipts,old_task_model,old_graph_model)
            ...
            visual_summary['scalar/task_learning_rate'] = model.task_optimizer.param_groups[0]['lr']
            visual_summary['scalar/graph_learning_rate'] = model.graph_optimizer.param_groups[0]['lr']
            ...
            if model.iteration % model.engine_cfg['save_iter'] == 0:
                ...
                if model.engine_cfg['with_test'] and (model.iteration % model.engine_cfg['test_iter'] == 0):
                    model.msg_mgr.log_info("Running test...")
                    model.eval()
                    result_dict = BaseModellifelong.run_test(model)
                    model.train()

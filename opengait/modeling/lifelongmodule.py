...
class Gaitpart_Graph(nn.Module):
    def __init__(self, hidden_dim, input_dim, sigma=2.0, proto_graph_vertex_num=None, meta_graph_vertex_num=128):
        super(Gaitpart_Graph, self).__init__()
        ...
        self.meta_GCN = GraphConvolution(self.hidden_dim)
    ...
    def forward(self, inputs):
        #inputs [n ,c, p]
        inputs_n,inputs_c,inputs_p = inputs.shape
        inputs = inputs.transpose(-2,-1).contiguous().view(inputs_n*inputs_p, inputs_c)
        ...
        protos = inputs
        meta_graph = self._construct_graph(self.meta_graph_vertex, self.meta_graph_vertex)
        proto_graph = self._construct_graph(protos, protos)
        ...
        rep = representation[0:batch_size].view(inputs_n, inputs_p, inputs_c).transpose(-2,-1)

        return rep, [correlation_meta,correlation_transfer_meta, correlation_protos]

    def _construct_graph(self, A, B):
        m = A.size(0)
        n = B.size(0)
        I = torch.eye(n, requires_grad=False).to(torch.device('cuda'))

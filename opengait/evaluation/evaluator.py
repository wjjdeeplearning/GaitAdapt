...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    gallery_seq_dict = {'HID2022': ['gallery'],
    ...
    gallery_view_list = view_list.copy()
    probe_view_list = view_list.copy()
    if dataset == 'HID2022':
        gallery_view_list =['5','6','7','8','9']
        probe_view_list = ['0', '1', '2', '3', '4']
        view_num = 5
    num_rank = 1
    # TSNE
    # nt,ct,pt = feature.shape[0],feature.shape[1],feature.shape[2]
    # print(nt,pt,ct)
    # X = feature.transpose(0,2,1).reshape((nt*pt, ct))
    # X_embedded = TSNE(n_components=3, init="pca",n_iter=1000, learning_rate=100).fit_transform(X)
    # feature = X_embedded.reshape((nt,pt,3)).transpose(0,2,1)
    ...
    # print(seq_type)
    # print(view)
    # colors = {'075': {'nm-01':'black','nm-02':'black','nm-03':'black','nm-04':'gray','nm-05':'gray','nm-06':'gray','bg-01':'red','bg-02':'green','cl-01':'purple','cl-02':'yellow'},
    #           '076': {'nm-01':'blue','nm-02':'blue','nm-03':'blue','nm-04':'blue','nm-05':'blue','nm-06':'blue','bg-01':'blue','bg-02':'blue','cl-01':'blue','cl-02':'blue'},
    #           '077': {'nm-01':'blue','nm-02':'blue','nm-03':'blue','nm-04':'blue','nm-05':'blue','nm-06':'blue','bg-01':'blue','bg-02':'blue','cl-01':'blue','cl-02':'blue'},
    #           '078': {'nm-01':'blue','nm-02':'blue','nm-03':'blue','nm-04':'blue','nm-05':'blue','nm-06':'blue','bg-01':'blue','bg-02':'blue','cl-01':'blue','cl-02':'blue'},
    #           '079': {'nm-01':'blue','nm-02':'blue','nm-03':'blue','nm-04':'blue','nm-05':'blue','nm-06':'blue','bg-01':'blue','bg-02':'blue','cl-01':'blue','cl-02':'blue'}
    #           }
    # color = []
    # for i in range(len(label)):
    #     color.append(colors[label[i]][seq_type[i]])
    # figure = plt.figure()
    ...
    # plt.show()
    # feature = np.expand_dims(X_embedded, axis=2)
    # feature = X_embedded
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        for (v1, probe_view) in enumerate(probe_view_list):
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)
            probe_x = feature[pseq_mask, :]
            ...
            # all_gallery = None
            # all_gallery_y = None
            for (v2, gallery_view) in enumerate(gallery_view_list):
                ...
                #     all_gallery = gallery_x
                #     all_gallery_y = gallery_y
                # else:
                #     all_gallery = np.append(all_gallery,gallery_x,axis=0)
                #     all_gallery_y = np.append(all_gallery_y,gallery_y)

                #
                # if has_probe == False:
                #     probe_x = reducer.fit_transform(probe_x[:, :, 0])
                #     probe_x = np.expand_dims(probe_x, axis=2)
                #     has_probe = True
                # gallery_x = reducer.transform(gallery_x[:, :, 0])
                # gallery_x = np.expand_dims(gallery_x, axis=2)
                ...
                dist = cuda_dist(probe_x, gallery_x, metric)
                # print(gallery_y)
                # gy = gallery_y[dist.topk(5, largest=False)[1].cpu().numpy()]
                # unique_elements = [np.unique(row, return_counts=True)[0] for row in gy]
                # counts = [np.unique(row, return_counts=True)[1] for row in gy]
                #
                # t = np.empty((0,))
                #
                ...
                #     # print(u)
                #     # print(np.argmax(c))
                #     t = np.append(t, u[np.argmax(c)])

                idx = dist.topk(num_rank, largest=False)[1].cpu().numpy()
                # print(np.reshape(probe_y, [-1, 1]) == gallery_y[idx])
                # print(idx)
                # for n,i in enumerate(probe_y):
                #     # print(i)
                #     # print(n)
                #     x = int(i) - 75
                #     y = int(gallery_y[idx[n]])-75
            ...
            # color = []
            # for i in range(len(all_gallery_y)):
            #     color.append(colors[all_gallery_y[i]])
            # figure = plt.figure()
            # pca.fit(all_gallery[:,:,0])
            # all_gallery = pca.transform(all_gallery[:,:,0])
            # all_gallery = tsne.fit_transform(all_gallery[:, :, 0])
            # plt.scatter(all_gallery[:, 0], all_gallery[:, 1], color=color)  # 绘制散点图。
            #
            #
            # plt.show()
            # gallery_x = np.expand_dims(gallery_x, axis=2)
    ...
    # np.savetxt('output.txt', confusion_matrix, delimiter=',')
    ...
    out_str = ""
    for type_ in probe_seq_dict[dataset].keys():
        sub_acc = de_diag(acc[type_], each_angle=False)
...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    ...
    if dataset not in ('CASIA-B', 'OUMVLP', 'CASIA-E', 'HID2022'):
...
def HID_submission(data, dataset, rerank=True, metric='euc'):
    ...
    probe_mask = (label == "probe2")
    gallery_mask = (label != "probe2")
    ...
    if rerank:
        ...
        re_rank = re_ranking(
            dist, probe_x.shape[0], k1=4, k2=4, lambda_value=0.3)
